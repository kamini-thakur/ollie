...
class Pyre_Latest_Foot_Featured_Media_Widget extends WP_Widget {
	...
	{
		$widget_ops = array('classname' => 'pyre_homepage_media-product-feat', 'description' => 'Highlight content above footer');
		...
		parent::__construct('pyre_homepage_media-widget-product-feat', 'Home: "Get Frover" Widget', $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance)
	{
		...
		<div class="footer-highlight-home <?php echo $args['widget_id']; ?><?php if($checkbox_pro): ?> light-fonts-pro<?php endif; ?>" <?php if($widget_bg): ?>style="background-color:<?php echo $widget_bg; ?>;"<?php endif; ?>>
			<div class="width-container">
				<?php if($title): ?>
					<h1 class="footer-highlight-widget"><?php echo $title; ?></h1>
				<?php endif; ?>
				
				<?php if($summary_text): ?>
						<div class="summary-text-pro"><?php echo do_shortcode( $summary_text ) ?></div>
				<?php endif; ?>
				
				<?php if($link_text): ?><a href="<?php echo $link_link; ?>" class="progression-button secondary-button"><?php echo $link_text; ?><?php if($link_icon): ?><i class="ls-sc-button-icon-right fa <?php echo do_shortcode( $link_icon ) ?>"></i><?php endif; ?></a><?php endif; ?>
					
				<?php if($secondary_link_text): ?><a href="<?php echo $secondary_link_link; ?>" class="progression-button"><?php echo $secondary_link_text; ?><?php if($secondary_link_icon): ?><i class="ls-sc-button-icon-right fa <?php echo do_shortcode( $secondary_link_icon ) ?>"></i><?php endif; ?></a><?php endif; ?>
	...
	function form($instance)
	{
		...
		$defaults = array('title' => 'GET FROVER', 'summary_text' => '', 'link_text' => 'All Features', 'link_link' => '#', 'secondary_link_text' => '', 'secondary_link_link' => '', 'widget_bg' => '#f4f5f5', 'widget_bg_img' => '', 'widget_image_left_pro' => '', 'checkbox_pro' => '1', 'link_icon' => 'fa-list-alt', 'secondary_link_icon' => 'fa-credit-card');
		...
		<p>
			<label for="<?php echo $this->get_field_id('secondary_link_text'); ?>"><?php _e( 'Secondary Button Text', 'progression' ); ?>:</label>
			<input class="widefat" style="width: 216px;" id="<?php echo $this->get_field_id('secondary_link_text'); ?>" name="<?php echo $this->get_field_name('secondary_link_text'); ?>" value="<?php echo $instance['secondary_link_text']; ?>" />
		</p>
		
		<p>
			<label for="<?php echo $this->get_field_id('secondary_link_link'); ?>"><?php _e( 'Secondary Button Link', 'progression' ); ?>:</label>
			<input class="widefat" style="width: 216px;" id="<?php echo $this->get_field_id('secondary_link_link'); ?>" name="<?php echo $this->get_field_name('secondary_link_link'); ?>" value="<?php echo $instance['secondary_link_link']; ?>" />
		</p>
		
		<p>
			<label for="<?php echo $this->get_field_id('secondary_link_icon'); ?>"><?php _e( 'Secondary Button Icon', 'progression' ); ?>:</label>
			<input class="widefat" style="width: 216px;" id="<?php echo $this->get_field_id('secondary_link_icon'); ?>" name="<?php echo $this->get_field_name('secondary_link_icon'); ?>" value="<?php echo $instance['secondary_link_icon']; ?>" />
		</p>
